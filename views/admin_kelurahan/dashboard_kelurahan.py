...
class DashboardKelurahan(ctk.CTkFrame):
    ...
    def _show_main(self):
        ...
        # ── Filter Bar ──
        filter_card = ctk.CTkFrame(c, fg_color="transparent")
        filter_card.pack(fill="x", padx=30, pady=(0, 10))

        ctk.CTkLabel(filter_card, text="Status:",
                     font=ctk.CTkFont(size=12, weight="bold")).pack(side="left", padx=(0, 5))
        self.filter_var = ctk.StringVar(value="Semua")
        ctk.CTkComboBox(
            filter_card, values=["Semua", "Menunggu", "Diproses Kelurahan", "Selesai", "Ditolak"],
        ...
        ctk.CTkLabel(filter_card, text="Dari:",
                     font=ctk.CTkFont(size=12, weight="bold")).pack(side="left", padx=(0, 5))
        self.date_from_var = ctk.StringVar(value="")
        self.date_from_entry = ctk.CTkEntry(
            filter_card, textvariable=self.date_from_var,
        ...
        ctk.CTkButton(
            filter_card, text="📅", width=32, height=32, corner_radius=8,
        ...
        ctk.CTkLabel(filter_card, text="Sampai:",
                     font=ctk.CTkFont(size=12, weight="bold")).pack(side="left", padx=(0, 5))
        self.date_to_var = ctk.StringVar(value="")
        self.date_to_entry = ctk.CTkEntry(
            filter_card, textvariable=self.date_to_var,
            width=110, height=32, corner_radius=8,
            placeholder_text="YYYY-MM-DD", font=ctk.CTkFont(size=11),
            state="readonly"
        )
        ...
        ctk.CTkButton(
            filter_card, text="🔍", width=40, height=32, corner_radius=8,
        ...
        ctk.CTkButton(
            filter_card, text="Reset", width=60, height=32, corner_radius=8,
    ...
    def _show_date_picker(self, target_var):
        ...
        try:
            ...
            return  # tkcalendar not installed, use manual input
    ...
    def _show_detail_page(self, laporan: dict):
        ...
        build_detail_header(
            ...
            breadcrumb="Dashboard Kelurahan  ›  Detail"
        )
        ...
        # Tentukan apakah laporan masih bisa diaksi oleh Kelurahan
        ACTIONABLE_STATUSES = {"Menunggu", "Diproses Kelurahan"}
        ...
        if not is_actionable:
            ...
            status_labels = {
                "Diproses Kecamatan": "diteruskan ke Kecamatan",
            ...
            ctk.CTkLabel(
                notice,
                text=f"⚠️  Laporan ini sudah {keterangan}. Tidak dapat diubah dari level Kelurahan.",
                font=ctk.CTkFont(size=12),
    ...
    def _proses(self, status_baru: str):
        ...
        action_labels = {
            'Diproses Kelurahan': 'memproses laporan ini di tingkat Kelurahan',
            'Selesai': 'menandai laporan ini sebagai SELESAI',
            'Ditolak': 'MENOLAK laporan ini',
            'Diproses Kecamatan': 'mengeskalasi laporan ini ke tingkat Kecamatan',
        }
        action_icons = {
            'Diproses Kelurahan': '🔄',
            'Selesai': '✅',
            'Ditolak': '❌',
            'Diproses Kecamatan': '⬆️',
        }
        action_colors = {
            'Diproses Kelurahan': '#1E88E5',
            'Selesai': '#43A047',
            'Ditolak': '#E53935',
            'Diproses Kecamatan': '#7B1FA2',
        }
